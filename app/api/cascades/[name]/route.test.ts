 ...
 *
 * PUT accepts { name?: string (rename), entries?: [...] (replace entries) }.
...
describe('Cascades API — item', () => {
  beforeEach(() => {
    ...
    services.cascades.save({ name: 'fleet', entries: ENTRIES, sourceHarness: 'h_a' })
  })
  afterAll(() => {
  ...
  it('PUT {entries} replaces the entries', async () => {
    const next = [{ provider: 'openai', model: 'gpt-5' }]
    const res = await PUT(req('PUT', { entries: next }), makeParams('fleet'))
    expect(res.status).toBe(200)
    expect((await res.json()).entries).toEqual(next)
    expect(services.cascades.get('fleet')?.entries).toEqual(next)
  })
  ...
  it('PUT {name} renames; the old name is gone', async () => {
    ...
    expect(services.cascades.get('fleet-v2')?.entries).toEqual(ENTRIES)
  })

  it('PUT {name, entries} does both', async () => {
    const next = [{ provider: 'openai', model: 'gpt-5' }]
    const res = await PUT(req('PUT', { name: 'renamed', entries: next }), makeParams('fleet'))
    expect(res.status).toBe(200)
    const rec = await res.json()
    expect(rec.name).toBe('renamed')
    expect(rec.entries).toEqual(next)
  ...
  it('PUT on a missing cascade is 404', async () => {
    const res = await PUT(req('PUT', { entries: ENTRIES }), makeParams('ghost'))
